...
(Figure~\ref{fig:feedback-noise}, $M=200$) shows the shape of that degradation, and the
benefit of the conflict-mask cleanup pass introduced in Section~2.1. Accuracy is perfect up
to $35\%$ of engram bits flipped, then declines smoothly -- not catastrophically -- over a
narrow window ($35$--$48\%$) before settling at chance. The cleanup pass consistently rescues
accuracy inside this transition window (e.g.\ $63.1\%$ vs.\ $49.0\%$ at $42\%$ noise;
$95.8\%$ vs.\ $88.9\%$ at $40\%$), exactly where it matters -- outside this window either the
query is already easy (no rescue needed) or already unrecoverable (nothing to rescue).

\begin{figure}[h]
\centering
\includegraphics[width=0.75\linewidth]{figures/hippocampus_feedback_noise_clean.png}
\caption{Digit-cell retrieval accuracy vs.\ query noise, with and without the conflict-mask
cleanup pass. Degradation is graceful (a narrow transition window, not a cliff), and cleanup
measurably rescues accuracy specifically inside that window.}
