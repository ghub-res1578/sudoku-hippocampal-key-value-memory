...
The comparison is double-edged: a paired nearest-neighbor baseline shows our pipeline is not
yet more accurate than trivial pixel-space classification at small stored-set sizes, an honest
limitation we report rather than omit; but on the specific property Vector-HaSH is designed to
provide, our architecture succeeds where classical and even pseudo-inverse-trained Hopfield
networks -- the latter using the identical learning rule we do -- collapse to zero recovery an
order of magnitude sooner, isolating the separate error-correcting attractor, not the learning
rule, as the source of the graceful degradation. All code, data, and reproduction instructions
...
most instructive comparison, because it is the same rule ours uses: it is algebraically
\emph{exact} at zero noise up to at least $M{=}200$, but its basins of attraction shrink so
fast under even modest noise that $5\%$ bit-flip recall falls from $100\%$ ($M{=}100$) to
$25\%$ ($M{=}150$) to $0\%$ ($M{=}200$) -- tracing out, in noise tolerance rather than raw
capacity, a Hopfield-style cliff of its own.

\refstepcounter{table}\label{tab:hopfield}
\noindent\textbf{Table \thetable.} Exact recovery vs.\ $M$ on identical MNIST images (moderate matched-severity noise:
$5\%$ bit-flip for the three Hopfield variants, $\sigma{=}0.5$ Gaussian pixel noise for ours;
not the same units, see Section~\ref{sec:hop}).
\begin{center}
\begin{tabular}{lccccc}
\toprule
...
\begin{center}
\includegraphics[width=\linewidth]{figures/hopfield_vs_our_network_comparison.png}
\end{center}
\refstepcounter{figure}\label{fig:hopfield}
\noindent\textbf{Figure \thefigure.} Left: exact recovery vs.\ $M$, all four methods, fixed moderate noise. Right: exact
recovery vs.\ noise at fixed $M{=}50$ (chosen below every method's zero-noise collapse point),
two separate noise axes since the corruption models differ.


Our network is the only one of the four that never reaches zero in the tested range, holding
$10$--$27\%$ recovery out to $M{=}800$ -- an order of magnitude past where the rule it shares
with the pseudo-inverse baseline already fails completely. Because that baseline uses the
identical learning rule, this difference isolates the source of the graceful decline: not the
pseudo-inverse rule itself, but the presence of a separate, content-specific error-correcting
step (the Sudoku-legality attractor) layered on top of it. This is the piece Vector-HaSH's true
scaffold provides in a content-\emph{independent} way (Section~2); ours is content-dependent
(legality-specific) but still delivers the qualitative signature the scaffold is meant to
produce, and does so an order of magnitude beyond where the shared learning rule alone can
reach.
...
The cross-modal extension has its own, separately reported limitations. It is not yet more
accurate than a trivial raw-pixel nearest-neighbor classifier at $M{=}200$
(Section~\ref{sec:knn-baseline}) -- whatever value the architecture offers over brute-force
exemplar search has to come from properties we have not yet tested at the scale where they
would matter (large $M$, where storing and searching raw exemplars becomes impractical but a
fixed-size engram remains cheap; or fusing partial pixel and partial relational cues, which
nearest-neighbor over images alone cannot do). The ridge-regularization fix in
Section~\ref{sec:capacity-hetero} was verified only up to $M{=}2000$; whether a single,
$M$-independent regularization schedule holds arbitrarily far past that, or whether $\lambda$
must keep growing with $M$, is untested. Finally, the Hopfield comparison uses a bit-flip
noise model for the three auto-associative baselines and Gaussian pixel noise for ours
(Table~\ref{tab:hopfield}); these are not numerically equivalent corruption scales, so the
comparison should be read as a qualitative contrast in failure \emph{shape} (smooth decline
that never reaches zero, vs.\ collapse to exactly zero within a narrow range), not a
claim of matched-severity superiority.
...
ridge pseudo-inverse ($\lambda=10^{-4}$ default, $\lambda=0.1$--$1.0$ where noted for
larger $M$), images normalized to $[0,1]$ and Gaussian noise added and reclipped to that
range. MNIST/Fashion-MNIST are $28\times28$ (dimension $784$); CIFAR-100 is converted to
grayscale by averaging RGB channels ($32\times32$, dimension $1024$). Baseline (Section~
\ref{sec:knn-baseline}): paired McNemar exact test (\texttt{scipy.stats.binomtest} on
discordant-pair counts) between our pipeline's and nearest-neighbor's exact-match outcome on
identical noisy queries. Hopfield baselines (Section~\ref{sec:hop}): images bipolarized by
thresholding at $0.5$; classical rule is the outer-product/Hebbian sum with zero diagonal;
covariance rule subtracts each pixel's own mean activity (not a single global scalar) before
the same sum; pseudo-inverse rule reuses the identical dual-form solver as $W_{SI}$/$W_{IS}$
with the pattern set as both input and target. Synchronous recall update
$s\leftarrow\mathrm{sign}(Ws)$ preserves a neuron's current state on an exact tie (net input
$0$) rather than forcing it to $+1$, since mean-centered patterns can have exactly-zero
weights on pixels that are constant across every stored image.
