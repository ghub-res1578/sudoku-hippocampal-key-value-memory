...
\begin{table}[h]
\centering
\caption{Collapse-noise threshold, essentially flat across a 200-fold range of $M$
...
\caption{Digit-cell retrieval accuracy vs.\ query noise, with and without the conflict-mask
...
\caption{Unknown-cell accuracy vs.\ Gaussian drive-current noise (clue fraction $0.30$),
...
\caption{Representative cleanup examples (one dataset per row-group): noisy input, image
...
\begin{table}[h]
\centering
\caption{Exact grid recovery vs.\ image noise, three datasets ($M{=}200$, $n{=}200$/point,
Wilson 95\% CI).}
\label{tab:hetero-datasets}
\begin{tabular}{lccc}
\toprule
...
\caption{Top-left: our pipeline vs.\ the nearest-neighbor baseline (paired). Top-right: the
...
\label{fig:capacity-consolidation}
\end{figure}
...
\begin{table}[h]
\centering
\caption{Exact recovery vs.\ $M$ on identical MNIST images (moderate matched-severity noise:
...
\caption{Left: exact recovery vs.\ $M$, all four methods, fixed moderate noise. Right: exact
