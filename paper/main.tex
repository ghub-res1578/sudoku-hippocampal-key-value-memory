...
Sections~\ref{sec:graceful-noise} and \ref{sec:emergent} both test robustness to
\emph{missing} information (query bit-flips; unknown cells). We separately tested robustness
to \emph{corrupted} information specific to the spiking mechanism: rather than any cell being
entirely unknown, every known cell's drive current itself was perturbed with Gaussian jitter
before driving the network (clue fraction fixed at $0.30$; unknown cells still use the
associative engram's own guess, unperturbed; $n=20$ trials/point, paired across noise
levels). Figure~\ref{fig:drive-noise} shows the result: accuracy declines smoothly as
